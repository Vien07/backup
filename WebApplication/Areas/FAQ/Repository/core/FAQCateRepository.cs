...
{
    public class FAQCateRepository : IFAQCateRepository
    {
        ...
        public FAQCateRepository(DBContext dbContext, IFileServices fileSrv, ICommonServices common, IMemoryCache memoryCache)
        {
        ...
        public dynamic LoadData(SearchDto search)
        {
            ...
            _memoryCache.Remove(ConstantStrings.CacheFAQCateName + "vi");
            _memoryCache.Remove(ConstantStrings.CacheFAQCateName + "en");

            return new { Data = rs, Paging = Paging };
        }
        public void GetAll(ref List<dynamic> list, long? parentId = null, string prefix = "")
        {
            var data = (from a in _dbContext.FAQCates
            ...
            foreach (var item in data)
            {
                ...
                d.CountPost = (from a in _dbContext.FAQCate_FAQDetails
                               where a.FAQCatePid == item.Pid
                               join b in _dbContext.FAQDetails on a.FAQDetailPid equals b.Pid
        ...
        public bool Enable(long[] Pid, bool active)
        {
            try
            {
                foreach (var item in Pid)
                {
                    try
                    {
                        var model = _dbContext.FAQCates.Where(p => p.Pid == item).FirstOrDefault();
        ...
        public dynamic Delete(int Pid)
        {
            try
            {
                ...
                var model = _dbContext.FAQCates.Where(p => p.Pid == Pid && p.isLocked == false).FirstOrDefault();
                var temmpCate = _dbContext.FAQCate_FAQDetails.Where(p => p.FAQCatePid == Pid).ToList();
                var temmpDetail = _dbContext.FAQDetails.Where(p => p.Deleted == false).ToList();

                var list = (from p in temmpCate
                            join e in temmpDetail
                              on p.FAQDetailPid equals e.Pid
                            select new
                            {
                                Pid = p.FAQDetailPid,
                            }).ToList();
                ...
                _dbContext.FAQCates.Remove(model);
                _dbContext.SaveChanges();
        ...
        public dynamic DeleteAll(int Pid)
        {
            try
            {
                ...
                //var list = _dbContext.FAQDetails.Where(p => p.FAQCatePid == Pid && p.Deleted==false).ToList();
                foreach (var item in list)
                {
                    var temp = _dbContext.FAQDetails.Where(p => p.Pid == item.Pid).FirstOrDefault();
        ...
        public dynamic Delete(long[] Pid)
        {
            try
            {
                foreach (var item in Pid)
                {
                    try
                    {
                        var model = _dbContext.FAQCates.Where(p => p.Pid == item && p.isLocked == false).FirstOrDefault();
                        var temmpCate = _dbContext.FAQCate_FAQDetails.Where(p => p.FAQCatePid == item).ToList();
                        var temmpDetail = _dbContext.FAQDetails.Where(p => p.Deleted == false).ToList();
        ...
        public dynamic Edit(long Pid)
        {
            try
            {
                ...
                var data = _dbContext.MultiLang_FAQCates.Where(p => p.FAQCatePid == Pid).ToList();
                var detail = _dbContext.FAQCates.Where(p => p.Pid == Pid).FirstOrDefault();

                List<dynamic> listData = new List<dynamic>();
                foreach (var item in data)
                {
                    var FAQ = _dbContext.FAQCates.Where(p => p.Pid == Pid).FirstOrDefault();
                    if (FAQ.Deleted != true)
                    {
                        ...
                        child.Pid = item.Pid;
                        child.FAQCatePid = item.FAQCatePid;
                        listData.Add(child);
                    }
                }
                return new
        ...
        public dynamic Insert(FAQCate faqCate, List<MultiLang_FAQCate> multiLang_faqCate)
        {
            using (var transaction = _dbContext.Database.BeginTransaction())
            {
                ...
                try
                {
                    ...
                    if (faqCate.ParentId != 0)
                    {
                        recurGetParentCate(ref arr, faqCate.ParentId);
                    }
                    var defaultData = multiLang_faqCate.Where(p => p.LangKey == DefaultLang).FirstOrDefault();
                    int maxOrder = _dbContext.FAQCates.Max(x => (int?)x.Order) ?? 1;
                    //faqCate.Pid = 0;

                    faqCate.Order = maxOrder + 1;
                    faqCate.ParentRoute = arr.Count > 0 ? string.Join("_", arr) : "";
                    _dbContext.FAQCates.Add(faqCate);
                    _dbContext.SaveChanges();
                    foreach (var item in multiLang_faqCate)
                    {
                        ...
                        var existSlug = (from a in _dbContext.FAQCates
                                         join b in _dbContext.MultiLang_FAQCates on a.Pid equals b.FAQCatePid
                        ...
                        item.FAQCatePid = faqCate.Pid;
                        _dbContext.MultiLang_FAQCates.Add(item);
                    }
        ...
        public dynamic Update(FAQCate faqCate, List<MultiLang_FAQCate> multiLang_faqCate)
        {
            using (var transaction = _dbContext.Database.BeginTransaction())
            {
                string messErr = "";
                ...
                try
                {
                    ...
                    var model = _dbContext.FAQCates.Where(p => p.Pid == faqCate.Pid).FirstOrDefault();


                    var childCateList = new List<long>();
                    var cateChildren = _dbContext.FAQCates.Where(p => p.ParentId == model.Pid && !p.Deleted && !p.isLocked).ToList();
                    ...
                    if (childCateList.Contains(faqCate.ParentId))
                    {
                    ...
                    model.Enabled = faqCate.Enabled;
                    model.ParentId = faqCate.ParentId;
                    model.UpdateDate = DateTime.Now;
                    model.ParentRoute = arr.Count > 0 ? string.Join("_", arr) : "";
                    foreach (var item in multiLang_faqCate)
                    {
                        var tempModel = _dbContext.MultiLang_FAQCates.Where(p => p.Pid == item.Pid).FirstOrDefault();
                        if (tempModel != null)
                        {
                ...
                try
                {
                    foreach (var item in Pid)
                    {
                        try
                        {
                            ...
                            FAQCate addFAQ = new FAQCate();
                            addFAQ.Order = maxOrder + 1;
                            addFAQ.Enabled = false;
                            _dbContext.FAQCates.Add(addFAQ);
                            _dbContext.SaveChanges();
                            List<MultiLang_FAQCate> detailModel = _dbContext.MultiLang_FAQCates.Where(p => p.FAQCatePid == model.Pid).ToList();
                            foreach (MultiLang_FAQCate itemDetail in detailModel)
                            {

                                MultiLang_FAQCate temp = new MultiLang_FAQCate();
                                ...
                                temp.FAQCatePid = addFAQ.Pid;
                                _dbContext.MultiLang_FAQCates.Add(temp);
                                _dbContext.SaveChanges();
                            }
        ...
        public bool Up(long Pid)
        {
            ...
            {
                long temp = 0;
                var data = _dbContext.FAQCates.Where(p => p.Pid == Pid).FirstOrDefault();
                temp = data.Order;
                var changeData = _dbContext.FAQCates.Where(p => p.Order > data.Order).OrderBy(p => p.Order).FirstOrDefault();
                if (changeData != null)
                {
        ...
        public bool Down(long Pid)
        {
            try
            {
                ...
                var data = _dbContext.FAQCates.Where(p => p.Pid == Pid).FirstOrDefault();
                temp = data.Order;
                var changeData = _dbContext.FAQCates.Where(p => p.Order < data.Order).OrderByDescending(p => p.Order).FirstOrDefault();
                data.Order = changeData.Order;
                changeData.Order = temp;
                _dbContext.SaveChanges();
                return true;
        ...
        public bool MoveRow(long from, long to)
        {
            try
            {
                var toRow = _dbContext.FAQCates.Where(p => p.Pid == to).FirstOrDefault();
                var fromRow = _dbContext.FAQCates.Where(p => p.Pid == from).FirstOrDefault();
                var max = _dbContext.FAQCates.Where(p => p.Deleted == false).Max(p => p.Order);
                ...
                if (fromRow.Order < toRow.Order)
                {
                    if (orderTo != max)
                    {
                        var listData = _dbContext.FAQCates.Where(p => p.Order < toRow.Order && p.Deleted == false && p.Pid != fromRow.Pid).ToList();
        ...
        public void recurGetParentCate(ref List<int> arr, int childId)
        {
            try
            {
                var parent = _dbContext.FAQCates.Where(x => !x.Deleted && x.Enabled && !x.isLocked && x.Pid == childId).FirstOrDefault();
                if (parent != null)
                {
                    arr.Add(childId);
        ...
        public void recurGetChildrenCate(ref List<long> arr, long parentId)
        {
            try
            {
                var cateChildren = _dbContext.FAQCates.Where(p => p.ParentId == parentId && !p.Deleted && !p.isLocked).ToList();
