...
{
    public class NewsCateRepository : INewsCateRepository
    {
        ...
        public NewsCateRepository(DBContext dbContext, IFileServices fileSrv, ICommonServices common, IMemoryCache memoryCache)
        {
        ...
        public dynamic LoadData(SearchDto search)
        {
            ...
            _memoryCache.Remove(ConstantStrings.CacheNewsCateName + "vi");
            _memoryCache.Remove(ConstantStrings.CacheNewsCateName + "en");

            return new { Data = rs, Paging = Paging };
        }
        public void GetAll(ref List<dynamic> list, long? parentId = null, string prefix = "")
        {
            var data = (from a in _dbContext.NewsCates
            ...
            foreach (var item in data)
            {
                ...
                d.CountPost = (from a in _dbContext.NewsCate_NewsDetails
                               where a.NewsCatePid == item.Pid
                               join b in _dbContext.NewsDetails on a.NewsDetailPid equals b.Pid
        ...
        public bool Enable(long[] Pid, bool active)
        {
            try
            {
                foreach (var item in Pid)
                {
                    try
                    {
                        var model = _dbContext.NewsCates.Where(p => p.Pid == item).FirstOrDefault();
        ...
        public dynamic Delete(int Pid)
        {
            try
            {
                ...
                var model = _dbContext.NewsCates.Where(p => p.Pid == Pid && p.isLocked == false).FirstOrDefault();
                var temmpCate = _dbContext.NewsCate_NewsDetails.Where(p => p.NewsCatePid == Pid).ToList();
                var temmpDetail = _dbContext.NewsDetails.Where(p => p.Deleted == false).ToList();

                var list = (from p in temmpCate
                            join e in temmpDetail
                              on p.NewsDetailPid equals e.Pid
                            select new
                            {
                                Pid = p.NewsDetailPid,
                            }).ToList();
                ...
                _dbContext.NewsCates.Remove(model);
                _dbContext.SaveChanges();
        ...
        public dynamic DeleteAll(int Pid)
        {
            try
            {
                ...
                //var list = _dbContext.NewsDetails.Where(p => p.NewsCatePid == Pid && p.Deleted==false).ToList();
                foreach (var item in list)
                {
                    var temp = _dbContext.NewsDetails.Where(p => p.Pid == item.Pid).FirstOrDefault();
        ...
        public dynamic Delete(long[] Pid)
        {
            try
            {
                foreach (var item in Pid)
                {
                    try
                    {
                        var model = _dbContext.NewsCates.Where(p => p.Pid == item && p.isLocked == false).FirstOrDefault();
                        var temmpCate = _dbContext.NewsCate_NewsDetails.Where(p => p.NewsCatePid == item).ToList();
                        var temmpDetail = _dbContext.NewsDetails.Where(p => p.Deleted == false).ToList();
        ...
        public dynamic Edit(long Pid)
        {
            try
            {
                ...
                var data = _dbContext.MultiLang_NewsCates.Where(p => p.NewsCatePid == Pid).ToList();
                var detail = _dbContext.NewsCates.Where(p => p.Pid == Pid).FirstOrDefault();

                List<dynamic> listData = new List<dynamic>();
                foreach (var item in data)
                {
                    var News = _dbContext.NewsCates.Where(p => p.Pid == Pid).FirstOrDefault();
                    if (News.Deleted != true)
                    {
                        ...
                        child.Pid = item.Pid;
                        child.NewsCatePid = item.NewsCatePid;
                        listData.Add(child);
                    }
                }
                return new
        ...
        public dynamic Insert(NewsCate newsCate, List<MultiLang_NewsCate> multiLang_newsCate)
        {
            using (var transaction = _dbContext.Database.BeginTransaction())
            {
                ...
                try
                {
                    ...
                    if (newsCate.ParentId != 0)
                    {
                        recurGetParentCate(ref arr, newsCate.ParentId);
                    }
                    var defaultData = multiLang_newsCate.Where(p => p.LangKey == DefaultLang).FirstOrDefault();
                    int maxOrder = _dbContext.NewsCates.Max(x => (int?)x.Order) ?? 1;
                    //newsCate.Pid = 0;

                    newsCate.Order = maxOrder + 1;
                    newsCate.ParentRoute = arr.Count > 0 ? string.Join("_", arr) : "";
                    _dbContext.NewsCates.Add(newsCate);
                    _dbContext.SaveChanges();
                    foreach (var item in multiLang_newsCate)
                    {
                        ...
                        var existSlug = (from a in _dbContext.NewsCates
                                         join b in _dbContext.MultiLang_NewsCates on a.Pid equals b.NewsCatePid
                        ...
                        item.NewsCatePid = newsCate.Pid;
                        _dbContext.MultiLang_NewsCates.Add(item);
                    }
        ...
        public dynamic Update(NewsCate newsCate, List<MultiLang_NewsCate> multiLang_newsCate)
        {
            using (var transaction = _dbContext.Database.BeginTransaction())
            {
                string messErr = "";
                ...
                try
                {
                    ...
                    var model = _dbContext.NewsCates.Where(p => p.Pid == newsCate.Pid).FirstOrDefault();


                    var childCateList = new List<long>();
                    var cateChildren = _dbContext.NewsCates.Where(p => p.ParentId == model.Pid && !p.Deleted && !p.isLocked).ToList();
                    ...
                    if (childCateList.Contains(newsCate.ParentId))
                    {
                    ...
                    model.Enabled = newsCate.Enabled;
                    model.ParentId = newsCate.ParentId;
                    model.UpdateDate = DateTime.Now;
                    model.ParentRoute = arr.Count > 0 ? string.Join("_", arr) : "";
                    foreach (var item in multiLang_newsCate)
                    {
                        var tempModel = _dbContext.MultiLang_NewsCates.Where(p => p.Pid == item.Pid).FirstOrDefault();
                        if (tempModel != null)
                        {
                ...
                try
                {
                    foreach (var item in Pid)
                    {
                        try
                        {
                            ...
                            NewsCate addNews = new NewsCate();
                            addNews.Order = maxOrder + 1;
                            addNews.Enabled = false;
                            _dbContext.NewsCates.Add(addNews);
                            _dbContext.SaveChanges();
                            List<MultiLang_NewsCate> detailModel = _dbContext.MultiLang_NewsCates.Where(p => p.NewsCatePid == model.Pid).ToList();
                            foreach (MultiLang_NewsCate itemDetail in detailModel)
                            {

                                MultiLang_NewsCate temp = new MultiLang_NewsCate();
                                ...
                                temp.NewsCatePid = addNews.Pid;
                                _dbContext.MultiLang_NewsCates.Add(temp);
                                _dbContext.SaveChanges();
                            }
        ...
        public bool Up(long Pid)
        {
            ...
            {
                long temp = 0;
                var data = _dbContext.NewsCates.Where(p => p.Pid == Pid).FirstOrDefault();
                temp = data.Order;
                var changeData = _dbContext.NewsCates.Where(p => p.Order > data.Order).OrderBy(p => p.Order).FirstOrDefault();
                if (changeData != null)
                {
        ...
        public bool Down(long Pid)
        {
            try
            {
                ...
                var data = _dbContext.NewsCates.Where(p => p.Pid == Pid).FirstOrDefault();
                temp = data.Order;
                var changeData = _dbContext.NewsCates.Where(p => p.Order < data.Order).OrderByDescending(p => p.Order).FirstOrDefault();
                data.Order = changeData.Order;
                changeData.Order = temp;
                _dbContext.SaveChanges();
                return true;
        ...
        public bool MoveRow(long from, long to)
        {
            try
            {
                var toRow = _dbContext.NewsCates.Where(p => p.Pid == to).FirstOrDefault();
                var fromRow = _dbContext.NewsCates.Where(p => p.Pid == from).FirstOrDefault();
                var max = _dbContext.NewsCates.Where(p => p.Deleted == false).Max(p => p.Order);
                ...
                if (fromRow.Order < toRow.Order)
                {
                    if (orderTo != max)
                    {
                        var listData = _dbContext.NewsCates.Where(p => p.Order < toRow.Order && p.Deleted == false && p.Pid != fromRow.Pid).ToList();
        ...
        public void recurGetParentCate(ref List<int> arr, int childId)
        {
            try
            {
                var parent = _dbContext.NewsCates.Where(x => !x.Deleted && x.Enabled && !x.isLocked && x.Pid == childId).FirstOrDefault();
                if (parent != null)
                {
                    arr.Add(childId);
        ...
        public void recurGetChildrenCate(ref List<long> arr, long parentId)
        {
            try
            {
                var cateChildren = _dbContext.NewsCates.Where(p => p.ParentId == parentId && !p.Deleted && !p.isLocked).ToList();
