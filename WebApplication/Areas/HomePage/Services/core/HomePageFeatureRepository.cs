...
{
    public class HomePageFeatureRepository : IHomePageFeatureRepository
    {
        ...
        public HomePageFeatureRepository(DBContext dbContext, IFileServices fileServices, ICommonServices common)
        {
            ...
            try
            {
                ...
                foreach (var item in listProjectPid)
                {
                    var temp = _dbContext.HomePages.Where(p => (p.Enabled == search.Enable || search.Enable == null)
                                                                && p.Deleted != true && p.Pid == item && p.Type == "feature").FirstOrDefault();
                    if (temp != null)
                    {
                        data.Add(temp);
                    }
                ...
                foreach (var item in data)
                {
                    ...
                    child.Images = UrlHomePageImages + item.Images;
        ...
        public bool Delete(int Pid)
        {
            try
            {
                ...
                _fileServices.DeleteFile(UrlHomePageImages, model.Images);
        ...
        public dynamic Edit(long Pid)
        {
            try
            {
                ...
                return new
                {
                    detail = new
                    {
                        picThumb = UrlHomePageImages + detail.Images,
        ...
        public dynamic Insert(Models.HomePage homepage, List<MultiLang_HomePage> multiLang_HomePage, IFormFile images)
        {
            using (var transaction = _dbContext.Database.BeginTransaction())
            {
                ...
                try
                {
                    ...
                    int maxOrder = _dbContext.HomePages.Where(x => x.Type == "feature").Max(x => (int?)x.Order) ?? 1;

                    homepage.Order = maxOrder + 1;
                    homepage.Type = "feature";

                    dynamic kt = _fileServices.SaveFileOriginal(images, UrlHomePageImages, defaultData.Title);
        ...
        public dynamic Update(Models.HomePage homepage, List<MultiLang_HomePage> multiLang_HomePage, IFormFile images)
        {
            using (var transaction = _dbContext.Database.BeginTransaction())
            {
                try
