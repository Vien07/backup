...
{
    public class FeatureCateRepository : IFeatureCateRepository
    {
        ...
        public FeatureCateRepository(DBContext dbContext, IFileServices fileSrv, ICommonServices common, IMemoryCache memoryCache)
        {
        ...
        public dynamic LoadData(SearchDto search)
        {
            ...
            _memoryCache.Remove(ConstantStrings.CacheFeatureCateName + "vi");
            _memoryCache.Remove(ConstantStrings.CacheFeatureCateName + "en");

            return new { Data = rs, Paging = Paging };
        }
        public void GetAll(ref List<dynamic> list, long? parentId = null, string prefix = "")
        {
            var data = (from a in _dbContext.FeatureCates
            ...
            foreach (var item in data)
            {
                ...
                d.CountPost = (from a in _dbContext.FeatureCate_FeatureDetails
                               where a.FeatureCatePid == item.Pid
                               join b in _dbContext.FeatureDetails on a.FeatureDetailPid equals b.Pid
        ...
        public bool Enable(long[] Pid, bool active)
        {
            try
            {
                foreach (var item in Pid)
                {
                    try
                    {
                        var model = _dbContext.FeatureCates.Where(p => p.Pid == item).FirstOrDefault();
        ...
        public dynamic Delete(int Pid)
        {
            try
            {
                ...
                var model = _dbContext.FeatureCates.Where(p => p.Pid == Pid && p.isLocked == false).FirstOrDefault();
                var temmpCate = _dbContext.FeatureCate_FeatureDetails.Where(p => p.FeatureCatePid == Pid).ToList();
                var temmpDetail = _dbContext.FeatureDetails.Where(p => p.Deleted == false).ToList();

                var list = (from p in temmpCate
                            join e in temmpDetail
                              on p.FeatureDetailPid equals e.Pid
                            select new
                            {
                                Pid = p.FeatureDetailPid,
                            }).ToList();
                if (list.Count() > 0)
                {
                ...
                _dbContext.FeatureCates.Remove(model);
                _dbContext.SaveChanges();
        ...
        public dynamic DeleteAll(int Pid)
        {
            try
            {
                ...
                //var list = _dbContext.FeatureDetails.Where(p => p.FeatureCatePid == Pid && p.Deleted==false).ToList();
                foreach (var item in list)
                {
                    var temp = _dbContext.FeatureDetails.Where(p => p.Pid == item.Pid).FirstOrDefault();
        ...
        public dynamic Delete(long[] Pid)
        {
            try
            {
                foreach (var item in Pid)
                {
                    try
                    {
                        var model = _dbContext.FeatureCates.Where(p => p.Pid == item && p.isLocked == false).FirstOrDefault();
                        var temmpCate = _dbContext.FeatureCate_FeatureDetails.Where(p => p.FeatureCatePid == item).ToList();
                        var temmpDetail = _dbContext.FeatureDetails.Where(p => p.Deleted == false).ToList();
        ...
        public dynamic Edit(long Pid)
        {
            try
            {
                ...
                var data = _dbContext.MultiLang_FeatureCates.Where(p => p.FeatureCatePid == Pid).ToList();
                var detail = _dbContext.FeatureCates.Where(p => p.Pid == Pid).FirstOrDefault();

                List<dynamic> listData = new List<dynamic>();
                foreach (var item in data)
                {
                    var Feature = _dbContext.FeatureCates.Where(p => p.Pid == Pid).FirstOrDefault();
                    if (Feature.Deleted != true)
                    {
                        ...
                        child.Pid = item.Pid;
                        child.FeatureCatePid = item.FeatureCatePid;
                        listData.Add(child);
                    }
                }
                return new
        ...
        public dynamic Insert(FeatureCate featureCate, List<MultiLang_FeatureCate> multiLang_featureCate)
        {
            using (var transaction = _dbContext.Database.BeginTransaction())
            {
                ...
                try
                {
                    ...
                    if (featureCate.ParentId != 0)
                    {
                        recurGetParentCate(ref arr, featureCate.ParentId);
                    }
                    var defaultData = multiLang_featureCate.Where(p => p.LangKey == DefaultLang).FirstOrDefault();
                    int maxOrder = _dbContext.FeatureCates.Max(x => (int?)x.Order) ?? 1;
                    //featureCate.Pid = 0;

                    featureCate.Order = maxOrder + 1;
                    featureCate.ParentRoute = arr.Count > 0 ? string.Join("_", arr) : "";
                    _dbContext.FeatureCates.Add(featureCate);
                    _dbContext.SaveChanges();
                    foreach (var item in multiLang_featureCate)
                    {
                        ...
                        var existSlug = (from a in _dbContext.FeatureCates
                                         join b in _dbContext.MultiLang_FeatureCates on a.Pid equals b.FeatureCatePid
                        ...
                        item.FeatureCatePid = featureCate.Pid;
                        _dbContext.MultiLang_FeatureCates.Add(item);
                    }
        ...
        public dynamic Update(FeatureCate featureCate, List<MultiLang_FeatureCate> multiLang_featureCate)
        {
            using (var transaction = _dbContext.Database.BeginTransaction())
            {
                string messErr = "";
                ...
                try
                {
                    ...
                    var model = _dbContext.FeatureCates.Where(p => p.Pid == featureCate.Pid).FirstOrDefault();


                    var childCateList = new List<long>();
                    var cateChildren = _dbContext.FeatureCates.Where(p => p.ParentId == model.Pid && !p.Deleted && !p.isLocked).ToList();
                    ...
                    if (childCateList.Contains(featureCate.ParentId))
                    {
                    ...
                    model.Enabled = featureCate.Enabled;
                    model.ParentId = featureCate.ParentId;
                    model.UpdateDate = DateTime.Now;
                    model.ParentRoute = arr.Count > 0 ? string.Join("_", arr) : "";
                    foreach (var item in multiLang_featureCate)
                    {
                        var tempModel = _dbContext.MultiLang_FeatureCates.Where(p => p.Pid == item.Pid).FirstOrDefault();
                        if (tempModel != null)
                        {
                ...
                try
                {
                    foreach (var item in Pid)
                    {
                        try
                        {
                            ...
                            FeatureCate addFeature = new FeatureCate();
                            addFeature.Order = maxOrder + 1;
                            addFeature.Enabled = false;
                            _dbContext.FeatureCates.Add(addFeature);
                            _dbContext.SaveChanges();
                            List<MultiLang_FeatureCate> detailModel = _dbContext.MultiLang_FeatureCates.Where(p => p.FeatureCatePid == model.Pid).ToList();
                            foreach (MultiLang_FeatureCate itemDetail in detailModel)
                            {

                                MultiLang_FeatureCate temp = new MultiLang_FeatureCate();
                                ...
                                temp.FeatureCatePid = addFeature.Pid;
                                _dbContext.MultiLang_FeatureCates.Add(temp);
                                _dbContext.SaveChanges();
                            }
        ...
        public bool Up(long Pid)
        {
            ...
            {
                long temp = 0;
                var data = _dbContext.FeatureCates.Where(p => p.Pid == Pid).FirstOrDefault();
                temp = data.Order;
                var changeData = _dbContext.FeatureCates.Where(p => p.Order > data.Order).OrderBy(p => p.Order).FirstOrDefault();
                if (changeData != null)
                {
        ...
        public bool Down(long Pid)
        {
            try
            {
                ...
                var data = _dbContext.FeatureCates.Where(p => p.Pid == Pid).FirstOrDefault();
                temp = data.Order;
                var changeData = _dbContext.FeatureCates.Where(p => p.Order < data.Order).OrderByDescending(p => p.Order).FirstOrDefault();
                data.Order = changeData.Order;
                changeData.Order = temp;
                _dbContext.SaveChanges();
                return true;
        ...
        public bool MoveRow(long from, long to)
        {
            try
            {
                var toRow = _dbContext.FeatureCates.Where(p => p.Pid == to).FirstOrDefault();
                var fromRow = _dbContext.FeatureCates.Where(p => p.Pid == from).FirstOrDefault();
                var max = _dbContext.FeatureCates.Where(p => p.Deleted == false).Max(p => p.Order);
                ...
                if (fromRow.Order < toRow.Order)
                {
                    if (orderTo != max)
                    {
                        var listData = _dbContext.FeatureCates.Where(p => p.Order < toRow.Order && p.Deleted == false && p.Pid != fromRow.Pid).ToList();
        ...
        public void recurGetParentCate(ref List<int> arr, int childId)
        {
            try
            {
                var parent = _dbContext.FeatureCates.Where(x => !x.Deleted && x.Enabled && !x.isLocked && x.Pid == childId).FirstOrDefault();
                if (parent != null)
                {
                    arr.Add(childId);
        ...
        public void recurGetChildrenCate(ref List<long> arr, long parentId)
        {
            try
            {
                var cateChildren = _dbContext.FeatureCates.Where(p => p.ParentId == parentId && !p.Deleted && !p.isLocked).ToList();
