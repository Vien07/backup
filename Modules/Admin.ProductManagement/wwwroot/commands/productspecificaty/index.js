class AdminProductSpecificaty_Index extends AdminProductSpecificaty {
    constructor(...args) {
        super(...args);
    }
    ReRegister() {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.response.isError) {
                    ...
                    //location.href = "/ProductSpecificaty/Index";
    ...
    GenerateAccessToken() {
        ...
        $.ajax({
            type: "GET",
            url: this.Controller + "GenerateAccessToken",
            beforeSend: function (xhr) {
                xhr.setRequestHeader("XSRF-TOKEN",
                    $('input:hidden[name="__RequestVerificationToken"]').val());
            ...
            success: function (data) {
                if (!data.response.isError) {
                    ...
                    Object.keys(data.response.data).forEach((key) => {
                        $(`#Website_MisaApiConfig input[name=${_this.capitalizeFirstLetter(key)}]`).val(data.response.data[key])
                    })
                } else {
                    _this._commonAdmin.ToastifyAlert("Lưu thất bại!!!", 'error')
                }
