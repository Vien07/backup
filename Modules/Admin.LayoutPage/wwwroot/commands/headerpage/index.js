class AdminHeaderPage_Index {
    constructor(model) {
        ...
        this.Controller = SteamSystem.VirtualFolder + "/HeaderPage/"
    }
    ReRegister() {
        InitDragDropTable()
    }
    GenerateHeaderHtml(pid)
    {
        let _this = this;

        $.ajax({
            type: "GET",
            url: _this.Controller+"GenerateHeaderHtml",
            ...
            success: function (response) {
                ...
                $("#txtHtml").val(response.res.data);
                $("#" + _this.ModalDataID).modal("show");
                //setTimeout(codemirror_txtHtml.setValue(response.response.data), 1000)
    ...
    UpdateHeaderHtml(pid)
    {
        let _this = this;
        _commonAdmin.AlertWarning(function (confirmed) {
            if (confirmed) {
                $.ajax({
                    type: "GET",
                    url: _this.Controller + "UpdateHeaderHtml",
                    beforeSend: function (xhr) {
                        xhr.setRequestHeader("XSRF-TOKEN",
                            $('input:hidden[name="__RequestVerificationToken"]').val());
                    },
                    ...
                    success: function (response) {
                        console.log(response)
                        if (!response.responseObj.isError) {
                            _this._commonAdmin.ToastifyAlert("Cập nhật thành công!!!", '')

                        } else {
                            _this._commonAdmin.ToastifyAlert("Cập nhật thất bại!!!", 'error')
        ...
        }, "Bạn có muốn cập nhật Header ?");

    }
    UpdatePreivewHeaderHtml(pid) {
        let _this = this;
        _commonAdmin.AlertWarning(function (confirmed) {
            if (confirmed) {
                $.ajax({
                    type: "GET",
                    url: _this.Controller + "UpdatePreviewHeaderHtml",
                    beforeSend: function (xhr) {
                        xhr.setRequestHeader("XSRF-TOKEN",
                            $('input:hidden[name="__RequestVerificationToken"]').val());
                    },
                    ...
                    success: function (response) {
                        console.log(response)
                        if (!response.responseObj.isError) {
                            $("#btnUpdateHeaderHtml").attr("onclick", `_action.UpdateHeaderHtml(${pid})`);
    ...
    RevertHeaderHtml()
    {
        ...
        $.ajax({
            type: "GET",
            url: _this.Controller +"RevertHeaderHtml",
            beforeSend: function (xhr) {
                xhr.setRequestHeader("XSRF-TOKEN",
                    $('input:hidden[name="__RequestVerificationToken"]').val());
            },
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.response.isError) {
                    ...
                    //location.href = "/HeaderPage/Index";
