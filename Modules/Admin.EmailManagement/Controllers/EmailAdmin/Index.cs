...
namespace Admin.EmailManagement.Controllers
{
    public partial class EmailAdminController 
    {
        ...
        public IActionResult Index()
        {
            ...
            _pageModel.EditModel = new Database.EmailAdmin();
        ...
        {
            _pageModel.List = _srv.GetList(search).Data;
            string list = _viewRender.RenderPartialViewToString(EmailAdminConstans.StaticPath.PartialView.Index_Table, _pageModel.List);
        ...
        {
            _pageModel.Search = new ParamSearch();
            var res = _repEmailAdmin.Enable(ids, isEnable,CreateUser);
            var listData = _viewRender.RenderPartialViewToString(EmailAdminConstans.StaticPath.PartialView.Index_Table, _srv.GetList(_pageModel.Search).Data);
        ...
        public JsonResult OnGetMove(int fromId, int toId)
        {
            ...
            var res = _repEmailAdmin.Move(fromId, toId);
            var listData = _viewRender.RenderPartialViewToString(EmailAdminConstans.StaticPath.PartialView.Index_Table, _srv.GetList(_pageModel.Search).Data);
            return new JsonResult(new { response = res, listData = listData });
        ...
        public JsonResult EnableUpdateOrder(ParamSearch search)
        {
            ...
            var res = _repEmailAdmin.EnableUpdateOrder();
            var listData = _viewRender.RenderPartialViewToString(EmailAdminConstans.StaticPath.PartialView.Index_Table, _srv.GetList(search).Data);
            return new JsonResult(new { response = res, listData = listData });
        ...
        public JsonResult OnGetUpdateOrder(int id, double order)
        {
            ...
            var res = _repEmailAdmin.UpdateOrder(id, order);
